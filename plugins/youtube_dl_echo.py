...
async def echo(bot, update):
    ...
    if update.from_user.id not in Config.AUTH_USERS:
        ...
        if str(update.from_user.id) in Config.ADL_BOT_RQ:
            ...
            process_max_timeout = round(Config.UPROCESS_MAX_TIMEOUT/60)
            present_time = round(Config.UPROCESS_MAX_TIMEOUT-(current_time - previous_time))
            Config.ADL_BOT_RQ[str(update.from_user.id)] = time.time()
            if round(current_time - previous_time) < Config.UPROCESS_MAX_TIMEOUT:
                ...
                if format_string is not None and not "audio only" in format_string:
                    ikeyboard = [
                        InlineKeyboardButton(
                            "VIDEO " + approx_file_size + format_string + " ",
                            callback_data=(cb_string_video).encode("UTF-8")),
                        InlineKeyboardButton(
                            "FILE " + approx_file_size + " " + format_ext + " ",
